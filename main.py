...
def execute():
    ...
    # 读取批次与批大小
    current_batch = int(os.getenv('CURRENT_BATCH', '0'))  # 没有就从第0批
    batch_size = int(os.getenv('BATCH_SIZE', '9'))        # 默认每批9个

    # 账号/密码列表
    user_list = [u for u in users.split('#') if u.strip()]
    passwd_list = [p for p in passwords.split('#') if p.strip()]
    ...
    # 关键改动：越界则回到第0批，继续循环执行
    if current_batch >= total_batches:
        print(f"当前批次 {current_batch} >= 总批次 {total_batches}，从第0批重新开始")
        current_batch = 0
    ...
    print(f"本次执行批次：{current_batch+1}/{total_batches}，账号范围：[{start_index}:{end_index})")
    ...
    if use_concurrent:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            exec_results = list(executor.map(
                lambda x: run_single_account(len(batch_user_list), x[0], *x[1]),
                enumerate(zip(batch_user_list, batch_passwd_list))
            ))
    else:
        idx = 0
        for user_mi, passwd_mi in zip(batch_user_list, batch_passwd_list):
            exec_results.append(run_single_account(len(batch_user_list), idx, user_mi, passwd_mi))
            idx += 1
            if idx < len(batch_user_list):
                time.sleep(sleep_seconds)  # 串行场景下的间隔，脚本已有该变量

    # 汇总与推送
    success_count = sum(1 for r in exec_results if r.get('success'))
    summary = f"\n执行账号总数{len(batch_user_list)}，成功：{success_count}，失败：{len(batch_user_list) - success_count}"
    print(summary)
    push_to_push_plus(exec_results, summary)

    # 告知下次应执行的批次号（由工作流去写回仓库变量）
    os.environ['CURRENT_BATCH'] = str(current_batch + 1)
